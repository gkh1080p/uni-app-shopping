      ...
      <view class="goods">
        <navigator hover-class="none" class="navigator" v-for="goods in item.goodsItems.items" :key="goods.id"
          ...
          <view class="price">
            ...
            <text class="number">{{ goods.orderNum }}</text>
          </view>
        </navigator>
      </view>
...
onLoad((el) => {
  query.value = el?.type
  currHotMap = hotMap.find(item => item.type === query.value)
...
const getHotRecommendData = async () => {
  let res = await getHotRecommendAPI(currHotMap!.url, {
    // 通过环境变量，开发环境，修改初始页面方便测试分页结果
